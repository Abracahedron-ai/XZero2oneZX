...
WATCH_PATHS = [
    "D:/Zero2oneZ/src/agents",  # Agent scripts
    "D:/Zero2oneZ/agents",      # Generated agent scripts (legacy)
...
class DiscoveryBus:
    ...
    def __init__(self):
        ...
        self.running = False
        self.failed_endpoints: Dict[str, bool] = {}  # Track failed endpoints to reduce noise
                ...
                try:
                    async with self.session.get(url, timeout=5) as resp:
                        if resp.status == 200:
                            ...
                            # Mark endpoint as working if it was previously failed
                            if url in self.failed_endpoints:
                                del self.failed_endpoints[url]
                                print(f"[Discovery] MCP endpoint {url} is now available")
                except Exception as e:
                    # Only log error once per endpoint to reduce noise
                    if url not in self.failed_endpoints:
                        print(f"[Discovery] MCP endpoint {url} is not available (will retry silently)")
                        self.failed_endpoints[url] = True
    ...
    async def trigger_discovery(self):
        ...
        for url in MCP_DISCOVERY_URLS:
            try:
                ...
                # Only log if endpoint was previously working
                if url not in self.failed_endpoints:
                    print(f"[Discovery] Error during manual discovery for {url}: {e}")
