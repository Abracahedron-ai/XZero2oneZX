...
class EmotionFusion:
    ...
    def add_omnivinci_result(self, emotion: str, confidence: float, arousal: float, valence: float):
        """Add OmniVinci (multimodal context-aware) result."""
        self.omnivinci_history.append({
            "emotion": emotion,
            "confidence": confidence,
            "arousal": arousal,
            "valence": valence,
    ...
    def fuse(self) -> Optional[EmotionState]:
        """
        Fuse SER, FER, and OmniVinci results with EMA smoothing and confidence weighting.
        
        Returns:
            Unified emotion state or None if no data available
        """
        if not self.ser_history and not self.fer_history and not self.omnivinci_history:
            return None
        
        # Get latest results
        ser_latest = self.ser_history[-1] if self.ser_history else None
        fer_latest = self.fer_history[-1] if self.fer_history else None
        omnivinci_latest = self.omnivinci_history[-1] if self.omnivinci_history else None
        ...
        if omnivinci_latest:
            weight = self.omnivinci_weight * omnivinci_latest["confidence"]
            arousal += omnivinci_latest["arousal"] * weight
            valence += omnivinci_latest["valence"] * weight
            total_weight += weight
            sources["omnivinci"] = omnivinci_latest["confidence"]
        ...
        total_sources_weight = (
            self.ser_weight + self.fer_weight + self.omnivinci_weight
        )
        confidence = (
            (sources.get("ser", 0.0) * self.ser_weight) +
            (sources.get("fer", 0.0) * self.fer_weight) +
            (sources.get("omnivinci", 0.0) * self.omnivinci_weight)
        ) / total_sources_weight if sources else 0.0
