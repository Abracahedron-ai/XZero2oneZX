...
class FacebookModelSwarm:
    ...
    def _process_with_model(self, node: ModelNode, input_data: Any, model_type: Optional[str]) -> Any:
        ...
        if model_type == 'audio':
            ...
            # Vision processing (MetaCLIP, DINOv2, etc.)
            import numpy as np
            ...
            if hasattr(outputs, 'last_hidden_state'):
                ...
                return list(outputs.values())[0].cpu().numpy() if outputs else None
    ...
    def extract_3d_geometry(self, image: Any) -> Dict[str, Any]:
        """
        Extract 3D geometry features from image using DINOv2 or MetaCLIP.
        
        Args:
            image: Input image (PIL Image or numpy array)
            
        Returns:
            Dictionary with 3D geometry features and spatial understanding
        """
        import numpy as np
        
        # Find DINOv2 or MetaCLIP model
        dinov2_node = None
        for node in self.nodes:
            if 'dinov2' in node.model_id.lower() or 'dinov2' in node.model_path.lower():
                dinov2_node = node
                break
            elif 'metaclip' in node.model_id.lower() or 'metaclip' in node.model_path.lower():
                dinov2_node = node
                break
        
        if not dinov2_node:
            # Fallback to first vision-capable model
            dinov2_node = self.nodes[0] if self.nodes else None
        ...
        # Extract features
        try:
            features = self._process_with_model(dinov2_node, image, model_type='vision')
            
            # Format as 3D geometry representation
            if isinstance(features, np.ndarray):
                if len(features.shape) > 1:
                    # Average pool if needed
                    features = features.mean(axis=0) if features.ndim > 1 else features.flatten()
                ...
                return {
                    ...
                    "model_id": dinov2_node.model_id
                }
            else:
                return {
                    "features": np.zeros((768,)),
                    "spatial_understanding": "error",
                    "geometry_type": "unknown"
                }
        except Exception as e:
            print(f"[ModelSwarm] Error extracting 3D geometry: {e}")
            return {
                "features": np.zeros((768,)),
                "spatial_understanding": "error",
                "geometry_type": "unknown"
